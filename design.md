...
One thing the format spec gets wrong for that book is worth knowing. `sectioned-prose` states that bare numbers in the running text are manuscript page numbers, which holds for the Nag Hammadi and Codex Tchacos gospels but not here: the Akhmim run carries James's own verse numbers inline, alongside cross-references to the Greek. So whether a text's inline numbers are page markers is declared per book in `LIBRARY_BOOKS` rather than inferred from its format, and the Apocalypse of Peter's are marked as neither.
